...
var structTemplate = template.Must(template.New("structTemplate").Parse(`
type {{ .Name }} struct {
	{{- range $, $field := .Fields }}
...
func MakeHandlers(wr io.Writer, sp spec.Spec) error {
	for _, path := range sp.Paths {
		for _, method := range path.Methods() {
			if method != nil {
				if err := MakeHandler(wr, method); err != nil {
					return err
				}
			}
		}
...
func MakeHandler(wr io.Writer, path *spec.PathSpec) error {
	...
	for _, d := range defs {
		if typeDef, ok := d.(translator.TypeDef); ok {
			fmt.Println("level", typeDef.Level)
	...
	for _, d := range defs {
		if typeDef, ok := d.(translator.TypeDef); ok {
			err = MakeConstructor(wr, typeDef)
			if err != nil {
				return err
			}
		}
